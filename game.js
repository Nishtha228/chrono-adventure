...
if(world.id==="town"&&p.slide){p.vy=3;p.slide-=dt} if(world.id==="neon"&&p.freeze>0){p.vx*=.98}
...
function drawLevel(){ctx.save();if(settings.shake&&shake>0)ctx.translate((Math.random()-.5)*shake,(Math.random()-.5)*shake);drawWorld();const targetCamX=Math.max(0,Math.min(level.width-innerWidth,p.x-innerWidth*.4));cam.x+=((targetCamX-cam.x)*.14);cam.y=0;
